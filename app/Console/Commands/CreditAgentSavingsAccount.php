...
class CreditAgentSavingsAccount extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'agent:credit';
     ...
     * @var string
     */
    protected $description = 'Credits Agent Savings accounts.';
    ...
    {
        $accounts = Saving::on('mysql::write')->where('type', 'Agent')->get();
        foreach($accounts as $account){
            if($account->admin_setup){
                $nextSave = '';
                ...
                if($nextTime == $todaysTime) {
                    ...
                    $members = AgentSavings::on('mysql::write')->where('account_id', $account->id)->get();
                    ...
                    foreach ($members as $member) {
                        if($member->mid_payment != 1){
                            ...
                            if($amountToSave <= $walletBalance){
                                ...
                                $memBal = intval($member->balance) + $amountToSave;
                ...
                foreach ($members as $member) {
                    $nextSave = '';
                    if(strtoupper($member->cycle) == 'DAILY'){
                        $nextSave = '1 day';
                    }elseif (strtoupper($member->cycle) == 'WEEKLY'){
                        $nextSave = '1 week';
                    }elseif (strtoupper($member->cycle) == 'MONTHLY'){
                        $nextSave = '1 month';
                    }

                    //Check if the next payment is due today
                    $nextDate = $member->next_save;
                    $today = date('Y-m-d');
                    ...
                    if($nextTime == $todaysTime){
                        if($member->mid_payment != 1){
                            $user = User::on('mysql::write')->find($member->member_id);
                            $walletBalance = intval($user->wallet->balance);
                            $amountToSave = intval($member->amount);
                        ...
                        $member->update(['next_save'=>date('Y-m-d', strtotime($nextDate.$nextSave))]);
                    }
                }
            }
        }
